...
//struct node2d *mkExConfig (); the default configuration (we will not use this configuration in part 4)
...
// new functions added
struct node2d *mkExConfig2(); //the configuration for part 4
struct node2d * searchNode2d (struct node2d *p2dRoot, char *n2dName);
struct node1d * insert1dInOrder(struct node2d *p2dRoot, char *listName, char* nodeName);
...
int main (int argc, char *argv[]) {
    ...
    struct node2d *p2d;
    //p2d = mkExConfig();//the default configuration given by the sample code (we will not use it in part 4)
    p2d = mkExConfig2();//the configuration for part 4 of the lab
    ...
    while(fgets(oneLineCommand, 1202, stdin)!=NULL){
        ...
        struct node1d *p1dPointer;
        p1dPointer = insert1dInOrder(p2d,TwoDNodeName,OneDNodeName);
...
// functions in this file
/* we will not this default configuration in part 4
...
// create the same 2d nodes as before in mkExConfig but no 1d nodes and perform the insertions alphabetically
struct node2d *mkExConfig2() {
    struct node2d *p2d;
    p2d = mk2dNode(NULL, "R2D2", NULL);
    p2d = mk2dNode(NULL, "alice", p2d);
    p2d = mk2dNode(NULL, "joe", p2d);

    /* We do not need all those 1d nodes in this configuration
    insert1dInOrder(p2d, "joe", "xy2");
    insert1dInOrder(p2d, "joe", "sally");
    insert1dInOrder(p2d, "joe", "e342");
    insert1dInOrder(p2d, "R2D2", "cso");
    insert1dInOrder(p2d, "R2D2", "c3p0");
    */
    return p2d;
}
...
struct node1d *insert1dInOrder(struct node2d *p2dRoot, char *listName, char* nodeName){
    ...
    if(n2d->first==NULL){
        ...
        while(temp->next!=NULL && strcmp((temp->next)->name,nodeName)<0){
            temp = temp->next;
        }
        if(temp->next==NULL){
            temp->next = p1d;
